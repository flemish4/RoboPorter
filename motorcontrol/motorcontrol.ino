...
int prevleftSerialSpeed = 0 ;
int prevrightSerialSpeed = 0 ;
...
void leftSensorISR () {
  if(digitalRead(leftSensorPin_B) == digitalRead(leftSensorPin_A)) {
    leftSensorPIDCount --;
...
void rightSensorISR () {
  if(digitalRead(rightSensorPin_A) == digitalRead(rightSensorPin_B)) {
    rightSensorPIDCount ++;
...
void BatteryCurrentSensor1() {
  ...
  // Serial.print("mA");
  // Serial.print("\n");
  watts1 = BatteryAmps1 * batteryVoltage1;
